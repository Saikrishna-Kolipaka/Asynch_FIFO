...
( 
	...
	asynch_fifo_mem #(.D_WIDTH(D_WIDTH), .A_DEPTH(A_DEPTH)) asynch_mem (.wr_clk(wr_clk), .rd_clk(rd_clk), .wr_rstn(wr_rstn), .wr_en(mem_wr_en), .wr_addr(wr_addr), .rd_addr(rd_addr), .wr_data(wr_data), .rd_data(rd_data));
